...
def mock_agent_nodes():
    ...
    """
    with patch("src.state.AgentState") as mock_state, patch(
        "src.agents.index_matcher"
    ) as m_idx, patch("src.agents.tool_call_node") as m_tc, patch(
        "src.agents.tool_router"
    ) as m_tr, patch(
        "src.agents.summarizer_node"
    ) as m_sum, patch(
        "src.agents.formatter_node"
    ) as m_fmt, patch(
        "src.chain.stock_picker"
    ) as m_sp, patch(
        "src.chain.tool_call_node_stock_picker"
    ) as m_tc_sp, patch(
        "src.chain.formatter_node_stock_picker"
    ) as m_fmt_sp, patch(
        "src.chain.tool_router_stock_picker"
    ) as m_tr_sp, patch(
        "src.chain.stock_picker_summarizer"
    ) as m_sum_sp, patch(
        "src.chain.portfolio_optimizer"
    ) as m_po, patch(
        "src.chain.tool_call_node_portfolio_optimizer"
    ) as m_tc_po, patch(
        "src.chain.tool_router_portfolio_optimizer"
    ) as m_tr_po, patch(
        "src.chain.summarizer_portfolio_optimizer"
    ) as m_sum_po, patch(
        "src.chain.formatter_node_portfolio"
    ) as m_fmt_po:
